...
export async function transcribeAudio(audioAsBase64: string, mimeType: string) {
  const response = await gemini.models.generateContent({
    ...
    contents: [
      {
        text: 'Trasncreva o áudio para português do Brasil. Seja preciso e natural na transcrição. Mantenha a pontuaçãoa dequada e divida o texto em parágrafos quando for apropriado.',
      },
      {
        inlineData: {
          mimeType,
...
export async function generateAnswer( question: string, transcriptions: string[] ) {
  ...
  - Se fro citar o contexto, utilize o termo "conteúdo da aula".
